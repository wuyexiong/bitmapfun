 ...
 * 
 * 这个类封装起来完成一些任意长时间运行的工作 => 当加载一个位图到一个ImageView。
 * 它可以处理的事情:使用的内存和磁盘缓存，工作在后台线程，并设置一个占位符图像。
 */
public abstract class ImageWorker {
    ...
    /**
     * 根据data参数加载图像到一个ImageView.
     * (重写 {@link ImageWorker#processBitmap(Object)} 定义处理逻辑.
     * 如果{@link ImageCache}已使用 {@link ImageWorker#setImageCache(ImageCache)}来设置了，内存和磁盘缓存将被使用。
     * 如果图像在内存缓存中发现，立即设置
     * 否则 {@link AsyncTask} 将会创建去异步加载bitmap
     * 
    ...
    /**
     * 
     * 这个类是为了迎合作者自己构造的一个图片url数组构成的adapter而写，也是一样调用{@link ImageWorker#loadImage(Object data, ImageView imageView)} 这个方法来加载图片
    ...
    /**
     * 当前imageview 没有在进行工作，或者当前任务被中止，就返回true
     * 如果当前的任务已经进入阻塞状态，是不可停止的，返回false
     * 
